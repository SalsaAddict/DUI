...
CREATE TABLE [COB] (
		...
		CONSTRAINT [PK_COB] PRIMARY KEY NONCLUSTERED ([Id]),
		CONSTRAINT [UQ_COB_Description] UNIQUE CLUSTERED ([Description])
	)
GO
...
CREATE TABLE [RiskCode] (
		...
		CONSTRAINT [FK_RiskCode_RiskCode] FOREIGN KEY ([TerrorismId]) REFERENCES [RiskCode] ([Id]),
...
UPDATE rc
SET [TerrorismId] = tc.[TerrorismId]
FROM [RiskCode] rc
 JOIN (VALUES
			(N'1', N'2T'),
			(N'2', N'2T'),
			...
			(N'Y9', N'2T'),
			(N'ZX', N'2T')
		) tc ([RiskCodeId], [TerrorismId]) ON rc.[Id] = tc.[RiskCodeId]
GO

SELECT * FROM [RiskCode]
GO
...
CREATE TABLE [Binder] (
		...
		CONSTRAINT [CK_Binder_SecondId] CHECK ([SecondId] != [LeadId]),
...
CREATE TABLE [BinderSectionUnderwriter] (
		...
		CONSTRAINT [CK_BinderSectionUnderwriter_Percentage] CHECK ([Percentage] BETWEEN 0 AND 1)
	)
GO
...
CREATE TABLE [ProgrammeBinder] (
		...
		CONSTRAINT [FK_ProgrammeBinder_BinderUnderwriter] FOREIGN KEY ([BinderId], [UnderwriterId]) REFERENCES [BinderUnderwriter] ([BinderId], [UnderwriterId])
	)
GO
